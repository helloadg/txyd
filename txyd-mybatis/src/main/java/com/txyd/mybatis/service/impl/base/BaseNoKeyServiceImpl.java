...
import com.txyd.mybatis.dao.base.BaseNoKeyDao;
import com.txyd.mybatis.service.base.BaseNoKeyService;
...
public abstract class BaseNoKeyServiceImpl<T> extends BaseImpl implements BaseNoKeyService<T> {
	...
	@Autowired
	private BaseNoKeyDao<T> baseNoKeyDao;

	@Override
	public int insert(T object) {
		return this.baseNoKeyDao.insert(object);
	}
	
	@Override
	public int insertNotNull(T object) {
		return this.baseNoKeyDao.insertNotNull(object);
	}
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				if(list.get(i) != null && list.get(i).size() > 0){
					count += this.baseNoKeyDao.insertBatch(list.get(i));
				}
			}
		}
		return count;
	...
	public Integer insertNotExists( Map<T, T> insert) {
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i) != null && list.get(i).size() > 0) {
					count += this.baseNoKeyDao.insertNotExists(list.get(i));
				}
			}
		}
		return count;
    ...
    public int update(T update, T where) {
    	return this.baseNoKeyDao.update(update,where);
    }

	@Override
	public int delete(T object) {
		return this.baseNoKeyDao.delete(object);
	}
	
	@Override
	public List<T> select(T object, LinkedHashMap<String,String> sort, Integer offset,  Integer limit) {
		return this.baseNoKeyDao.select(object,sort,  offset,  limit, null);
	}

	@Override
	public List<T> select(T object, LinkedHashMap<String,String> sort, Integer offset,  Integer limit ,Boolean isForUpdate) {
		return this.baseNoKeyDao.select(object, sort, offset, limit, isForUpdate);
	}

	@Override
	public List<T> select(T object, Integer offset, Integer limit) {
		return this.baseNoKeyDao.select(object, null, offset, limit, null);
	}
	@Override
	public List<T> select(T object, Integer offset, Integer limit, Boolean isForUpdate) {
		return this.baseNoKeyDao.select(object, null, offset, limit, isForUpdate);
	}
	
	@Override
	public List<T> select(T object, LinkedHashMap<String,String> sort) {
		return this.baseNoKeyDao.select(object, sort, null, null, null);
	}

	@Override
	public List<T> select(T object, LinkedHashMap<String,String> sort, Boolean isForUpdate) {
		return this.baseNoKeyDao.select(object, sort, null, null, isForUpdate);
	}

	@Override
	public List<T> select(T object) {
		return this.baseNoKeyDao.select(object, null, null, null, null);
	}

	@Override
	public List<T> select(T object, Boolean isForUpdate) {
		return this.baseNoKeyDao.select(object, null, null, null, isForUpdate);
	}

	@Override
	public int selectCount(T object) {
		return this.baseNoKeyDao.selectCount(object);
	}
