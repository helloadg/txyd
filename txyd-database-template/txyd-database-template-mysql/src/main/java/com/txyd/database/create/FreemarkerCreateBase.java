	...
	public static boolean createFile(JavaConfigBean jcb,Map<String, String> fileMap,FileType type){
		...
		String outModel="";
		{
			if(type==FileType.model){
				...
				outModel=jcb.getXmlPath().toLowerCase().replace(".", "/");				
			}else if(type==FileType.baseMybatisXml){
				outModel=jcb.getXmlBasePath().toLowerCase().replace(".", "/");				
			}
		}
		String fileSuffix=".java";
		{
			if(type == FileType.mybatisConfigXml 
					|| type == FileType.baseMybatisXml
					|| type == FileType.mybatisXml){
				fileSuffix=".xml";
			}else{
				fileSuffix=".java";
			}
