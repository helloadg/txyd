...
public class HttpClientUtil {
    ...
    public static String post(String url, Map<String, String> param, RequestConfig requestConfig) {
        ...
        if (null != param && param.size() > 0) {
            ...
            try {
                log.info("set utf-8 form param to httppost");
    ...
    public static String postImg(String fileServer,String imgFilePath){
        ...
        HttpPost postRequest = new HttpPost(fileServer);//Post Request to specified URL
        MultipartEntityBuilder builder = MultipartEntityBuilder.create();
        builder.setMode(HttpMultipartMode.BROWSER_COMPATIBLE);
        FileBody fileBody = new FileBody(new File(imgFilePath));
        builder.addPart(imgFilePath, fileBody);
        postRequest.setEntity(builder.build());//adding request param to post request
        try {
    ...
    /*public static void postImg(String fileServer,String imgFilePath){
    	PropertiesFileUtils profile = PropertiesFileUtils.getInstance();
    	String fileServer_url = profile.getPropertyValue("prop/application.properties", "fileServer_url");
    	String imgFilePath_url = profile.getPropertyValue("prop/application.properties", "imgFilePath_url");
        if(null==fileServer||fileServer==""){
            fileServer = fileServer_url;
        }
        if(null==imgFilePath||imgFilePath==""){
        	imgFilePath = imgFilePath_url;
        }
        String targetURL = null;// TODO 指定URL
	   File targetFile = null;// TODO 指定上传文件
	   
	   targetFile = new File(imgFilePath_url);
	   targetURL = fileServer_url; //servleturl
